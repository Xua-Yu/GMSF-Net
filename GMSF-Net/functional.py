...
def projmap(X):
    ...
    batch_size, channels, n_in, _ = X.shape
    temp = th.zeros(batch_size,channels,n_in,n_in,dtype=X.dtype,device=X.device)
    for i in range(batch_size):
        for j in range(channels):
            temp[i,j] = th.matmul(X[i,j],X[i,j].t())
    return temp

def projpool(X):

    batch_size, channels, n_in, n_out = X.shape
    temp = th.zeros(batch_size,1,n_in,n_out,dtype=X.dtype,device=X.device)
    for j in range(channels):
        for i in range(batch_size):
            temp[i, 0] = temp[i,j]+ temp[i,0]
            temp[i,0] = temp[i,0]/channels
    return temp
...
class ProjPoolLayer(nn.Module):
    ...
    def __init__(self):
        super().__init__()
    def forward(self, x):
        avgpool = th.nn.AvgPool2d(int(math.sqrt(4)))
        return avgpool(x)

class ProjPoolLayer_A(F):
    # AProjPooling  c/n ==0
    @staticmethod
    def forward(ctx, x, n=1):
        ...
        return th.cat(new_x, 1).reshape(b, new_c, h, w)

    @staticmethod
    def backward(ctx, dx):
        x = ctx.saved_variables
        n = 1
        return th.repeat_interleave(dx / n, 1, 1)
...
class Eig(F):
    ...
    def forward(ctx, P):
        ...
        for i in range(batch_size):
            for j in range(channels):
                ...
                S[i, j] = s
        #X = U.matmul(BatchDiag(S)).matmul(U.transpose(2, 3))
        ctx.save_for_backward(U,S,P)
        return U
    ...
    def backward(ctx, dx):
        ...
        U, S, P = ctx.saved_variables
        batch_size, channels, n, n = P.shape
        dp = th.zeros_like(U, device=dx.device)
        K = calcuK(S)
        print(K)
        for i in range(batch_size):
            for j in range(channels):
                Ut = U[i, j].t()
                mid_1 = K[i, j].t() * th.matmul(Ut, dx[i, j])
                mid_2 = th.matmul(U[i, j], mid_1)
                dp[i, j] = th.matmul(mid_2, Ut)
                #dp[i, j] = U[i, j] @ ( (U[i, j].t() @ dx[i, j]) @ U[i, j].t())
        return dp
...
class OrthmapFunction(F):
    ...
    def backward(ctx, dx):
        ...
        grad_output = th.cat((dx, pad_zero), 3)
        print(U)
        Ut = U.transpose(-1, -2)
        print(Ut)
        K = calcuK(S)
...
def frmap_channels(X,W):
    batch_size, channels_in, n_in, _ = X.shape
    channels_out, _, _, n_out = W.shape
    P = th.zeros(batch_size, channels_out, n_out, n_out, dtype=X.dtype, device=X.device)
    for co in range(channels_out):
        P[:, co, :, :] = sum([frmap(X[:, ci, :, :], W[co, ci, :, :]) for ci in range(channels_in)])
    return P
...
class reorth(F):
    ...
    def forward(ctx, P):
        batch_size, channels, n, n = P.shape  # batch size,channel depth,dimension
        Q, R = th.zeros_like(P, device=P.device), th.zeros_like(P,dtype=P.dtype, device=P.device)
        for i in range(batch_size):
            for j in range(channels):
                Q[i, j], R[i, j] = th.linalg.qr(P[i,j])
                Q[i,j] = th.matmul(Q[i,j], th.diag_embed(th.sign(th.sign(th.diagonal(R[i,j], dim1=0, dim2=1)) + 0.5)))
        ctx.save_for_backward(Q, R, P)
        return Q
    @staticmethod
    def backward(ctx, dx):
        # if __debug__:
        #     import pydevd
        #     pydevd.settrace(suspend=False, trace_only_current_thread=True)
        Q,R,P = ctx.saved_variables
        batch_size, channels, n, n = P.shape
        dp = dx
        for i in range(batch_size):
            for j in range(channels):
                S = th.eye(Q[i,j].shape[-2]) - th.matmul(Q[i,j], Q[i,j].transpose(-1, -2))
                mid_1 = th.matmul(Q[i,j].transpose(-1, -2), dx[i,j])
                mid_2 = th.tril(mid_1) - th.tril(mid_1.transpose(-1, -2))
                mid_3 = th.matmul(S.transpose(-1, -2), dx[i,j]) + th.matmul(Q[i,j], mid_2)

                mid_4 = th.inverse(R[i,j]).transpose(-1, -2)

                dp[i,j] = th.matmul(mid_3, mid_4)
        return dp
